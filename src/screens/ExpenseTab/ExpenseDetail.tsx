...
class ExpenseDetail extends Component<IDefaultNavProps, IState> {
    ...
    componentWillMount() {
        ...
        this.state.expense.balances.map((balance: Balance, index: number) => {
            console.log(balance);
            if (balance.amount > 0) {
                text.push(<Text key={index}>{balance.person.firstname} {balance.person.lastname} payed {currencies[this.state.expense.currency].symbol}{balance.amount}</Text>);
            } else {
                text.push(<Text key={index}>{balance.person.firstname} {balance.person.lastname} has to pay {currencies[this.state.expense.currency].symbol}{balance.amount}</Text>);
            }
        });
