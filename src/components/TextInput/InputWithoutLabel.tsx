            ...
            <View>
                <TextInput
                    ...
                    onChangeText={(text: any) => {
                        ...
                        this.setState({ input: text }, () => this.updateSuggestions());
                    }}
                    onBlur={() => {
                        if (this.props.onBlur) {
                            this.props.onBlur();
        ...
        if (this.props.options && this.state.input.trim() !== '') {
            this.props.options.map((option: string, index: number) => {
                if (option !== this.state.input || suggestions.length > 3) {
                    if (option.toLocaleLowerCase().includes(this.state.input.toLocaleLowerCase()) || StringSimilarity.compareTwoStrings(option, this.state.input) > 0.5) {
                        suggestions.push(
